import {
  ...
  type PostgresListItem,
  type PostgresListQuery,
} from "@repo/shared";
...
function mapListItem(row: {
  ...
  return {
    ...
    id: row.id,
    name: row.name,
    postgresVersion: row.postgresVersion as PostgresListItem["postgresVersion"],
    project: {
      id: row.projectId,
...
export async function listPostgresInstances(
  ...
  if (search) {
    ...
    filters.push(
      or(
        ...
        ilike(postgresInstances.postgresVersion, searchPattern),
        ilike(postgresInstances.lastHealthStatus, searchPattern),
      )!,
    );
  }
  ...
  const rows = await db
    .select({
      ...
      healthStatus: postgresInstances.lastHealthStatus,
      id: services.id,
      name: services.name,
      postgresVersion: postgresInstances.postgresVersion,
